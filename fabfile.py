#!/usr/bin/env python
# -*- coding: utf-8 -*-
# create by https://github.com/Jeremaihloo/megenerator at 2016-07-23 03:32:43.768197743 +0800 CST
...
def backup():
    ...
    f = 'backup-.-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick . > %s' % (db_user, db_password, f))
...
def restore2local():
    ...
    sqls = [
        ...
        'create database .;',
        'grant select, insert, update, delete on ..* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s . < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
